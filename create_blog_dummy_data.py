...
def create_admin_user(db: Session):
    ...
    admin = db.query(User).filter(
        (User.email == "admin@33kotidham.com") | 
    ...
    if not admin:
        admin = User(
            name="Blog Admin",
            email="blogadmin@33kotidham.com",  # Different email to avoid conflicts
            mobile="9999999998",  # Different mobile to avoid conflicts
        ...
        db.commit()
        db.refresh(admin)
        print(f"✅ Created new admin user: {admin.name}")
    else:
        print(f"✅ Using existing admin user: {admin.name}")
